  ...
  return (
    <div className="space-y-6">
      <div className="flex justify-between items-center">
        <div>
          <h1 className="text-2xl font-bold text-red-800">Orders</h1>
          ...
          <CardContent className="p-6">
            <div className="flex items-center">
              ...
              <div className="ml-4">
                ...
                <p className="text-2xl font-bold text-red-800">{orders.length}</p>
              </div>
            </div>
          </CardContent>
        </Card>
            ...
            <table className="w-full">
              <thead>
                <tr className="border-b border-red-200">
                  ...
                  <th className="text-left py-3 px-4 text-red-800">Items</th>
                  <th className="text-left py-3 px-4 text-red-800">Total</th>
              ...
              <tbody>
                {filteredOrders.map((order) => (
                  <tr key={order.id} className="border-b border-red-100 hover:bg-red-50">
                    <td className="py-3 px-4">
                      <div className="font-medium text-red-800">{order.orderNumber}</div>
                    </td>
                    <td className="py-3 px-4">
                      <div>
                        <div className="font-medium text-red-800">{order.customer}</div>
                        <div className="text-sm text-slate-600">{order.email}</div>
                      </div>
                    </td>
                    <td className="py-3 px-4 text-slate-600">{order.items}</td>
                    <td className="py-3 px-4 text-red-700">${order.total}</td>
                    <td className="py-3 px-4">
          ...
          <div className="bg-white rounded-xl shadow-2xl w-full max-w-3xl overflow-hidden">
            <div className="flex items-center justify-between px-6 py-4 border-b border-slate-200">
              <div>
                ...
                <h3 className="text-lg font-semibold text-red-800">{selectedOrder.orderNumber}</h3>
              </div>
              <Button
                variant="ghost"
                size="sm"
            ...
            <div className="px-6 py-5 space-y-4">
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <div className="bg-red-50/50 border border-red-100 rounded-lg p-4">
                  ...
                  <p className="text-sm font-semibold text-red-800">{selectedOrder.customer}</p>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
                <div className="border border-slate-200 rounded-lg p-4">
                  ...
                  <p className="text-lg font-semibold text-red-800">{selectedOrder.items}</p>
                </div>
                <div className="border border-slate-200 rounded-lg p-4">
                  <p className="text-xs uppercase tracking-wide text-slate-500 mb-1">Total</p>
                  <p className="text-lg font-semibold text-red-800">${selectedOrder.total.toFixed(2)}</p>
                </div>
                <div className="border border-slate-200 rounded-lg p-4">
                  <p className="text-xs uppercase tracking-wide text-slate-500 mb-1">Date</p>
                  <p className="text-lg font-semibold text-red-800">{selectedOrder.date}</p>
                </div>
              </div>

              <div className="border border-slate-200 rounded-lg p-4">
                <p className="text-sm font-semibold text-red-800 mb-2">Order Summary</p>
                <p className="text-sm text-slate-600">
                  This order contains <span className="font-semibold text-red-800">{selectedOrder.items}</span> items with a total value of{' '}
                  <span className="font-semibold text-red-800">${selectedOrder.total.toFixed(2)}</span>. Current status is{' '}
                  <span className="font-semibold text-red-800">{selectedOrder.status}</span>.
                </p>
              </div>
            </div>

            <div className="flex justify-end gap-2 px-6 py-4 border-t border-slate-200 bg-red-50/50">
              <Button
                variant="outline"
                className="hover:bg-red-50 hover:border-red-300"
                onClick={() => setSelectedOrder(null)}
