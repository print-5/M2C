...
export default function ReviewSubmit({ onPrev, onGoToStep, data }: ReviewSubmitProps) {
  ...
  const handleSubmit = (e?: React.FormEvent) => {
    ...
    if (!confirmChecked || !termsChecked || !privacyChecked) return;
    setIsSubmitted(true);
  };
    ...
    <div className="max-w-420 mx-auto py-8 px-4 animate-slide-up">
      ...
      <form onSubmit={handleSubmit} className="space-y-6">
        ...
        <div className="flex justify-between pt-4">
          <Button className='bg-red-500 text-white p-4 rounded-md' type="button" variant="outline" onClick={onPrev}>Back</Button>
          <Button className="bg-green-400 text-white p-4 rounded-md" type="submit" size="lg" disabled={!confirmChecked || !termsChecked || !privacyChecked} onClick={handleSubmit}>Submit for Approval</Button>
        </div>
      </form>
    </div>
  );
